...
為確保 NDH 能夠安全、可靠地進行系統升級，我們提供了完整的系統更新機制：
...
- **快速回滾**: Kubernetes 原生回滾 (< 5 分鐘)
- **從快照回滾**: 完整系統狀態復原 (< 15 分鐘)
- **自動回滾**: 當錯誤率 > 5% 時自動觸發

#### **數據庫遷移**

採用 **向後相容的遷移策略** (Expand-Migrate-Contract)：
1. **Expand**: 新增新欄位，保留舊欄位
2. **Migrate**: 遷移數據，雙寫模式
3. **Contract**: 移除舊欄位
