...
4. **智能分析引擎**：內建 MEP 設計、衝突檢測、流體分析等原生能力
...
3. **智能分析引擎**：提供 MEP 設計、衝突檢測、流體分析等原生能力
...
**職責**：
- 對 3D 模型進行精確的衝突檢測
- 識別不同專業（結構、MEP、建築）之間的碰撞點
- 提供衝突報告和視覺化

**技術實現**：

**多階段策略**：
1. **廣相檢測（Broad-Phase Detection）**：
   - 使用包圍盒層次結構（BVH）或八叉樹
   - 快速排除大部分不相交的物體對
   - 時間複雜度：O(n log n)

2. **窄相檢測（Narrow-Phase Detection）**：
   - 使用 GJK（Gilbert-Johnson-Keerthi）演算法
   - 判斷兩個凸體是否相交
   - 若相交，使用 EPA（Expanding Polytope Algorithm）計算穿透深度

**效能優化**：
- 空間分割（Spatial Partitioning）
- 並行計算（Multi-threading）
